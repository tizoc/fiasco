...
.. code-block:: ruby
   ...
   class MyReceiver # MyReceiver is a receiver
     # @route is a mapper object bound to the application and to MyReceiver
     @route = Fiasco::Mapper.bind(self, $app)

     @route['/home'] # Pushes a matcher to the stack, with one rule defined:
                     # the PATH has to be equal to '/home'
     def home # Defines a method on the receiver with one matcher on the stack.
       ...    # For each matcher on the stack, a mapping from the matcher to
     end      # this endpoint (MyReceiver#home) is registered with the
              # application object bound to the mapper.
   end

   # We register an instance of MyReceiver as a handler
   $app.add_handler(MyReceiver.new)
...
If no *mapping* that can match the request is found, then the status of the response is finished with a status code 404 assigned to it.
...
The captures are passed by name to the method handling the request. This means that if the order of arguments in the method declaration are different to the order of captured framents in the request PATH, the captures will sill be bound to the names expected.
